...
call_cuffdiff <- function(counts, normFactors, cond1, cond2){
	...
	for (i in seq(from=1, to=length(counts[,1]), by=1)){
		...
		if(cond1 == 1){
			...
			var_A <- counts[i,1:cond1] / normFactors[1:cond1] / cond1;
		} else{
			mean_A <- rowMeans(counts[i,1:cond1]);
			var_A <- rowMeans(counts[i,1:cond1] / normFactors[1:cond1]) / cond1;	
		}
		
		if(cond2 == 1){
			mean_B <- counts[i,(cond1+1):(cond1+cond2)];
			var_B <- counts[i,(cond1+1):(cond1+cond2)] / normFactors[(cond1+1):(cond1+cond2)] / cond2;
		} else{
			mean_B <- rowMeans(counts[i,(cond1+1):(cond1+cond2)]);
			var_B <- rowMeans(counts[i,(cond1+1):(cond1+cond2)] / normFactors[(cond1+1):(cond1+cond2)]) / cond2;
		}
